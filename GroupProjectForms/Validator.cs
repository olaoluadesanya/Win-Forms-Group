...
namespace GroupProjectForms
{
    public static class Validator
    {
        ...
        /// <summary>
        /// Checks if a textbox value is null or lesser than another textbox value
        /// </summary>
        /// <param name="textBox"></param>
        /// <param name="textBoxMax"></param>
        /// <returns></returns>
        public static bool IsNullOrLesserThan(TextBox textBox, TextBox textBoxMax)
        {
            //decimal number = (textBox.Text == "") ? Convert.ToDecimal(textBox.Text) : 0 ;
            decimal numberMax = Convert.ToDecimal(textBoxMax.Text);
            if (textBox.Text != "")
            {
                if (Convert.ToDecimal(textBox.Text) > numberMax)
                {
                    MessageBox.Show(textBox.Tag + " must be Null or less than " + textBoxMax.Tag  + ".", Title);
                    textBox.Focus();
                    return false;
                }
            }
